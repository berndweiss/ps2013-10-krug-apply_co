...
\begin{frame}
  ...
  \begin{itemize}
  \item Statistische Aggregation ($Dim_{input} > Dim_{output}$) vs
    Datentaufbereitung/-transformation ($Dim_{input} = Dim_{output}$)
  \item Ausgangsdatentyp (z.B. \code{data.frame}) vs Zieldatentyp (z.B. \code{vector})
  \item (Vektorisierte) Alternative zu Schleifen vs Aggregation/Transformation
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{small}
    ...
    \item \code{data.table}-Paket (meistens schnellste\footnote[frame]{Für einen
        ...
        2.10.2013} Lösung, aber eigenwillige Syntax)
    \item \ldots
    \end{itemize}
